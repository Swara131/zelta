...
export default function BillingPage() {
  ...
  const [checkoutProviders, setCheckoutProviders] = useState<CheckoutProviders | null>(
  ...
  const loadCheckoutProviders = useCallback(async (billingInterval: BillingInterval) => {

    try {

      const response = await fetch(

        `/api/billing/checkout/options?interval=${billingInterval}`
  ...
  const openUpgradeModal = useCallback(async () => {
    ...
    await loadCheckoutProviders(interval);
  ...
  useEffect(() => {
    ...
    void loadCheckoutProviders(interval);

  }, [interval, loadCheckoutProviders, showPaymentModal]);
  ...
  const startCheckout = async (provider: PaymentProvider) => {
    ...
    try {
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({ planId: "professional", interval }),
